        ...
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className={cn(
              ...
              "dark:bg-[var(--color-dark-card)] dark:border-[var(--color-dark-border)] dark:text-[var(--color-dark-text)]"
            )}
          />
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[var(--color-border)] dark:border-[var(--color-dark-border)]">
              {["Client", "Product", "Amount", "Source", "Status", "Date"].map(
                (h) => (
                  <th
                    key={h}
                    className="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-[var(--color-text-muted)] dark:text-[var(--color-dark-text-muted)]"
                  >
                    {h}
                  </th>
                )
          ...
          <tbody>
            {filtered.map((client) => (
              <tr
                key={client.id}
                className="border-b border-[var(--color-border)] dark:border-[var(--color-dark-border)] last:border-0 transition-colors hover:bg-[var(--color-bg-muted)] dark:hover:bg-[var(--color-dark-elevated)] cursor-pointer"
              >
                ...
                <td className="px-4 py-3 text-sm text-[var(--color-text-secondary)] dark:text-[var(--color-dark-text-secondary)]">
                  {client.product}
                </td>
